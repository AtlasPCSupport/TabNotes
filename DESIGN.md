---
version: alpha
name: TabNotes-linear-adaptation
description: "TabNotes dark product system adapted from Linear. Uses a deep graphite canvas (#090b12), cool ink text (#f3f7ff), and TabNotes electric blue (#2f6dff) as the single accent for actions, focus rings, and product highlights. Visual tone is technical, focused, and premium for a local-first notes workflow. Typography combines Space Grotesk for display hierarchy and Plus Jakarta Sans for UI/body readability, with IBM Plex Mono for keys and metadata. CTA voice is direct and utility-first: Open notes, Capture context, Keep it local."

colors:
  primary: "#2f6dff"
  on-primary: "#ffffff"
  primary-hover: "#5788ff"
  primary-focus: "#2b5be8"
  ink: "#f3f7ff"
  ink-muted: "#bdc9dd"
  ink-subtle: "#8f9cb3"
  ink-tertiary: "#67748c"
  ...
  surface-4: "#222b4c"
  hairline: "#2a3354"
  hairline-strong: "#3b4770"
  hairline-tertiary: "#4a588a"
  inverse-canvas: "#ffffff"
  inverse-surface-1: "#f5f6f6"
  inverse-surface-2: "#f6f7f7"
  inverse-ink: "#000000"
  brand-secure: "#4f6bb8"
  semantic-success: "#3ebd7d"
...
## TabNotes Brand Overrides

- **Brand Accent:** Use TabNotes electric blue (`#2f6dff`) as the only chromatic accent for primary actions, active pills, keyboard focus, and data highlights.
- **Typography Direction:**
  - Display/UI headings: `Space Grotesk` (600–700).
  - Body/controls: `Plus Jakarta Sans` (400–600).
  - Scoped keys / metadata / note IDs: `IBM Plex Mono`.
- **Logo & Imagery:** Keep the compact rounded-square `T` mark with subtle glow. Prefer product-first screenshots of Notes, Sidepanel, and Workspace flows over decorative gradients.
- **CTA Voice:** Utility-first and direct: `Open Notes`, `Capture Context`, `Stay Local`, `Load in Chrome`.
- **Interaction Personality:** Dense, fast, and calm. Use hairline borders, restrained motion, and high-contrast states rather than soft card-heavy decoration.

## Overview

Linear's marketing canvas is the deepest dark surface in this collection — `{colors.canvas}` is #010102, essentially pure black with a faint blue tint. On top sits a four-step surface ladder (`{colors.surface-1}` through `{colors.surface-4}`) for cards, panels, and lifted tiles, with hairline borders running from `{colors.hairline}` (#23252a) up through `{colors.hairline-strong}` and `{colors.hairline-tertiary}`. Light gray text (`{colors.ink}` #f7f8f8) carries the body and headlines.

The single chromatic accent is **Linear lavender-blue** `{colors.primary}` (#2f6dff) — used on the brand mark, focus rings, and the primary CTA button. A lighter hover state (`{colors.primary-hover}` #5788ff) and a focus-tinted variant (`{colors.primary-focus}` #2b5be8) extend the same hue. Linear avoids saturated greens, oranges, reds, etc. on the marketing canvas — the only semantic color is `{colors.semantic-success}` (#27a644) for status pills and the rare success indicator.
...
- **Lavender-blue brand accent** (`{colors.primary}` #2f6dff) — used scarcely on brand mark, focus, and the primary CTA.
...
- **Lavender Hover** ({colors.primary-hover}): Lighter lavender (#5788ff) — hovered state of the primary CTA.
- **Lavender Focus** ({colors.primary-focus}): Focus-ring tint (#2b5be8) — focused inputs, focused buttons.
- **Brand Secure** ({colors.brand-secure}): Muted lavender-gray (#4f6bb8) — used in "Linear Security" surfaces.
...
- **Space Grotesk** — Linear's custom display sans; fallback `SF Pro Display, -apple-system, system-ui, Segoe UI, Roboto`. Carries display-xl through subhead.
- **Plus Jakarta Sans** — Linear's custom text sans (a slightly different cut tuned for body sizes); same fallback stack. Carries body sizes, button labels, captions.
