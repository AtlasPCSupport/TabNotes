...
export default function SettingsPage() {
  ...
  const [canRestoreImport, setCanRestoreImport] = React.useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    useNotesStore.getState().load();
    hasRecoverySnapshot().then(setCanRestoreImport);
  }, [hasRecoverySnapshot]);

  const handleExport = async () => {
    const data = await exportStore();
    const completeData = { ...data, theme };
    const manual = createManualBackupEnvelope(completeData);
    const entered = window.prompt(
      'Optional: enter a password to encrypt this backup. Leave blank for a standard JSON backup.'
    );
    if (entered === null) return;
    const backup = entered
      ? await createEncryptedManualBackupEnvelope(manual, entered)
      : manual;
    const blob = new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `tabnotes-backup-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          (candidate as { kind?: unknown }).kind === 'tabnotes.encryptedBackup'
        ) {
          const entered = window.prompt('Enter the password for this encrypted backup:');
          if (entered === null) return;
          password = entered;
        }
      } catch {
        // The store reports invalid JSON with the standard import error.
      }
      await importData(text, password);
      setCanRestoreImport(await hasRecoverySnapshot());
      setImportStatus('Import successful! You can undo it below.');
    } catch {
      setImportStatus('Import failed — invalid file or password.');
    }
  ...
  const RadioBtn = ({
    ...
    <label
      ...
      style={{
        ...
        border: `1.5px solid ${current === value ? 'var(--color-accent)' : 'var(--color-border)'}`,
        background: current === value ? 'var(--color-accent-subtle)' : 'var(--color-bg)',
        cursor: 'pointer',
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-10)', maxWidth: 560 }}
    >
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          {SCOPE_OPTIONS.map((s) => (
            <RadioBtn
              ...
              onSelect={() => setDefaultScope(s.value)}
            />
          ))}
        </div>
      </Section>
      ...
      <Section title="Data">
        ...
        {canRestoreImport && (
          <button
            onClick={async () => {
              if (!window.confirm('Restore the data saved immediately before your last import?')) return;
              if (await restorePreImportSnapshot()) {
                setCanRestoreImport(false);
                setImportStatus('Pre-import data restored.');
              }
            }}
            style={{
              padding: 10,
        ...
        {importStatus && (
          <div
            style={{
              ...
              padding: 8,
              borderRadius: 'var(--radius-md)',
              background: importStatus.includes('success') || importStatus.includes('restored')
                ? 'var(--color-success-subtle)'
                : 'var(--color-danger-subtle)',
              color: importStatus.includes('success') || importStatus.includes('restored')
                ? 'var(--color-success)'
                : 'var(--color-danger)',
            }}
          >
      ...
      <Section title="Chrome Extension">
        ...
        >
          <p style={{ marginBottom: 'var(--space-3)' }}>
            The TabNotes Chrome extension brings contextual notes directly into your browser — one
            click away, always in context.
          </p>
          <a
      ...
      <Section title="Pro (Coming Soon)">
        ...
        >
          <div
            style={{ fontWeight: 700, fontSize: 'var(--text-md)', marginBottom: 'var(--space-2)' }}
          >
            ✦ TabNotes Pro
          </div>
          <div
